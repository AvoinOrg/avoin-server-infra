...
- `secondary/avoin-map-geocoding/` — Avoin Map geocoding front service. Exposes `GET /v1/search` via Traefik labels on `proxy-net`, forwards address/place queries to Pelias through `PELIAS_BASE_URL`, and optionally resolves exact Finnish estate-ID queries through the F004 PostGIS adapter when `ESTATE_LOOKUP_ENABLED=true` and the F003 sandbox estate tables are loaded.
- `secondary/pelias/` — Pelias geocoding stack: API + Elasticsearch + libpostal with profiled OpenStreetMap config/download/import jobs, OSM and NLS `Nimistö` dataset/import plans, and a documented Finnish official-data/Pelias CSV extension point. Only the API is exposed via Traefik labels on `proxy-net`.
...
- `secondary/plane/` — Plane project management board/tracker. Exposed via the Plane internal `proxy` service through Traefik labels on `proxy-net`; first-run admin, SMTP, and OIDC are completed in Plane `/god-mode`.
...
- `secondary/avoin-map-geocoding/` is stateless from the container's point of view. Configure `GEOCODING_DOMAIN`, `TRAEFIK_CERTRESOLVER`, `PELIAS_BASE_URL`, result limits, default Finland country/bbox values, timeout, and CORS origins in its `.env`. Exact estate-ID lookup is optional: keep `ESTATE_LOOKUP_ENABLED=false` until `secondary/estate-postgis-loader/LOAD-REPORT.md` records a successful sanitized F003 load, then provide the `ESTATE_POSTGIS_*` values through ignored `.env` or deployment secrets.
- `secondary/pelias/` includes a generated-config OSM import flow and Finnish official-data staging conventions for later custom Pelias CSV imports. Read `secondary/pelias/OSM-DATASET-IMPORT.md` for the Geofabrik Finland source decision, small-probe evidence, production command order, resource planning, cleanup, and rollback notes. Read `secondary/pelias/NLS-NIMISTO-DATASET-IMPORT.md` before adding NLS `Nimistö` place names; it records the source decision, API key handling, key-backed probe result, and Pelias CSV transform plan. Before expecting search results, render its Pelias config, create the schema, run the profiled OSM download/import jobs, and then start or restart the API. Full OSM/custom imports require operator-run commands plus enough local disk, memory, and time; generated config, raw source data, derived CSV, importer cache data, and Elasticsearch indexes stay under configured ignored data paths.
